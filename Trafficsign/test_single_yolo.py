...
def get_input_size(model, fallback=DEFAULT_SIZE):
    try: shape = model.input.shape
    except AttributeError: shape = model.input_shape
    return int(shape[1] if shape[1] is not None else fallback), int(shape[2] if shape[2] is not None else fallback)

def run_yolo_worker(image_path, conf, iou):
    worker = os.path.join(os.path.dirname(os.path.abspath(__file__)), "yolo_worker.py")
    if not os.path.isfile(worker): sys.exit(f"[Lỗi] Không tìm thấy {worker}")
    cmd = [sys.executable, worker, "--image", image_path, "--conf", str(conf), "--iou", str(iou)]
    res = subprocess.run(cmd, capture_output=True, text=True, timeout=120)
    if res.returncode != 0: sys.exit(f"[Lỗi] yolo_worker thất bại:\n{res.stderr.strip()}")
...
def draw_result(img_bgr, x1, y1, x2, y2, cls_id, conf_cls, yolo_conf, label, fallback=False):
    color = COLOR_HIGH if conf_cls >= 0.7 else COLOR_MEDIUM if conf_cls >= 0.45 else COLOR_LOW
    ...
    cv2.putText(img_bgr, text, (x1 + 2, ty - 2), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255,255,255), 1, cv2.LINE_AA)
    if not fallback:
        cv2.putText(img_bgr, f"Y:{yolo_conf:.2f}", (x1 + 2, y2 - 4), cv2.FONT_HERSHEY_SIMPLEX, 0.38, (200,200,200), 1, cv2.LINE_AA)

def main():
    args = parse_args()
    if not os.path.isfile(args.image): sys.exit(f"[Lỗi] Không tìm thấy ảnh: {args.image}")
    ...
    print(f"[YOLO] Phát hiện {n_det} biển báo." if n_det else "[YOLO] Dùng toàn bộ ảnh (fallback).")
    ...
    for idx, det in enumerate(detections):
        x1, y1, x2, y2, yolo_conf, fallback = det["x1"], det["y1"], det["x2"], det["y2"], det["yolo_conf"], det["fallback"]

        # === THUẬT TOÁN SQUARE PADDING ===
        if not fallback:
            bw, bh = x2 - x1, y2 - y1
        ...
        if args.normalize: crop_resized /= 255.0
        
        pred = clf_model.predict(np.expand_dims(crop_resized, 0), verbose=0)[0]
        top3 = np.argsort(pred)[::-1][:3]
        cls_id = int(top3[0])
        conf, label = float(pred[cls_id]), SIGNS.get(cls_id, "Unknown")

        region = "toàn ảnh (fallback)" if fallback else f"({x1},{y1})→({x2},{y2})"
        print(f"  Box {idx+1:<3} | {region:<25} | YOLO Conf: {yolo_conf:.2f}")
        print()
        
        for rank, cid in enumerate(top3, start=1):
            c_conf, c_label = float(pred[cid]), SIGNS.get(int(cid), "Unknown")
            bar = "#" * int(c_conf * 25)
    ...
    # Bật cửa sổ popup hiển thị ảnh kết quả
    try:
        ...
        print(f"\n[Cảnh báo] Không thể hiển thị cửa sổ ảnh (có thể do chạy trên server không có GUI): {e}")
